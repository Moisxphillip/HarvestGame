...
func interact(_type : String = "0")->void:
	...
	if plant == null :
		if !$Worms.active:           
			if _type == "Ype":
				#Hud.spend_money(1)
				Hud.get_node("UI/Bag").consume_item("Ype Seed")
				plant = ypeInstance.instantiate()
			
			elif _type == "Sunflower":
				#Hud.spend_money(2)
				Hud.get_node("UI/Bag").consume_item("Sunflower Seed")
				plant = sunflowerInstance.instantiate()
			
			elif _type == "Rose":
				#Hud.spend_money(3)
				Hud.get_node("UI/Bag").consume_item("Rose Seed")
		...
		if plant.name == "Rose":
			Hud.get_node("UI/Bag").add_item("rose")
		elif plant.name == "Sunflower":
			Hud.get_node("UI/Bag").add_item("sunflower")
		elif plant.name == "Ype":
			Hud.get_node("UI/Bag").add_item("ype")
